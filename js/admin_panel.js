function ispisi_tabelu(tabela, id) {
	...
	xhttp.open("GET", "http://diablo3.esy.es/admin/prikazi_tabelu/" + tabela + "/" + id, true);
	xhttp.send();
}
function obrisi_red(tabela, id, prva_kolona) {
	...
	xhttp.open("GET", "http://diablo3.esy.es/admin/obrisi_red/" + tabela + "/" + id + "/" + prva_kolona, true);
	xhttp.send();
}
...
$(function () {
	$(document).on("click", "button:has(.fa-floppy-o)", function () {
		if (this.id)
		{
			...
			var url = "http://diablo3.esy.es/admin/izmeni_red/" + id;
			...
			var url = "http://diablo3.esy.es/admin/dodaj_red/";
